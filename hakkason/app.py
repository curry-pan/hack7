...
app.config['DATASET_FOLDER'] = 'C:/hakk/hakk/datasets/testA'
app.config['RESULTS_FOLDER'] = 'C:/hakk/hakk/pytorch-CycleGAN-and-pix2pix/results'
...
def submit_form():
    if 'file' not in request.files or 'artist' not in request.form:
        return "ファイルまたはアーティストが選択されていません", 400
    ...
    if file:
        ...
        target_dir = 'C:/hakk/hakk/pytorch-CycleGAN-and-pix2pix'
        ...
        try:
            ...
            # 結果フォルダのクリア処理
            results_subfolder = os.path.join(app.config['RESULTS_FOLDER'], f'style_{artist}_pretrained', 'test_latest', 'images')
        ...
        # 変換後の画像をFlaskの静的ファイルディレクトリにコピー
        static_processed_image_path = os.path.join(processed_dir, f'{filename.split(".")[0]}_fake.png')
        shutil.copy(processed_image_path, static_processed_image_path)
